...
import { chevronIcon, filterContainer, conditionalTabletHide, conditionalMobileHide } from './Filter.css';

export const Filter: React.FunctionComponent<FilterProps> = (props) => {
  const { label, width, button, hideMobile, hideTablet } = props || {};

  return (
    <button style={{ width: width, borderRadius: button ? 45 : 8, justifyContent: button ? 'center' : 'space-between', marginLeft: button ? 8 : 4.8, paddingTop: button ? 0 : 1 }} className={`${filterContainer} ${hideMobile ? conditionalMobileHide : '' } ${hideTablet ? conditionalTabletHide : '' }`}>
      { label }
